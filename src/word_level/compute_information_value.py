...
def distances_to_information_value(distances, aggregation_func):
        ...
        for n in range(
            ...
            for k, v in sorted(distances_in_context.items()):
                ...
                if "_cross" in k:
                    ...
                    results[f"information_value_mean_{score_name}"].append(np.mean(_v))
                    results[f"information_value_min_{score_name}"].append(np.min(_v))
                    results[f"information_value_max_{score_name}"].append(np.max(_v))
                if "_self" in k:
                    score_name = k[: -len("_self")]
                    results[f"expected_information_value_{score_name}"].append(np.mean(_v))
                    results[f"information_value_deviation_{score_name}"].append(
                        abs(
                            results[f"information_value_mean_{score_name}"][-1]
                            - results[f"expected_information_value_{score_name}"][-1]
                        )
                    )
            ...
            for k, v in sorted(distances_out_of_context.items()):
                ...
                if "_cross" in k:
                    score_name = k[: -len("_cross")]
                    results[f"information_value_ooc_mean_{score_name}"].append(np.mean(_v))
                    results[f"information_value_ooc_min_{score_name}"].append(np.min(_v))
                    results[f"information_value_ooc_max_{score_name}"].append(np.max(_v))
                    results[f"context_informativeness_{score_name}"].append(
                        results[f"information_value_ooc_mean_{score_name}"][-1]
                        - np.mean(distances_in_context[k])
                    )
                if "_self" in k:
                    score_name = k[: -len("_self")]
                    results[f"expected_information_value_ooc_{score_name}"].append(np.mean(_v))
                    results[f"expected_context_informativeness_{score_name}"].append(
                        results[f"expected_information_value_ooc_{score_name}"][-1]
                        - np.mean(distances_in_context[k])
                    )
                    results[f"information_value_ooc_deviation_{score_name}"].append(
                        abs(
                            results[f"information_value_ooc_mean_{score_name}"][-1]
                            - results[f"expected_information_value_ooc_{score_name}"][-1]
                        )
            ...
            if args.random_contexts:
                for k, v in sorted(distances_in_random_context.items()):
                    ...
                    if "_cross" in k:
                        score_name = k[: -len("_cross")]
                        results[f"information_value_rnd_mean_{score_name}"].append(np.mean(_v))
                        results[f"information_value_rnd_min_{score_name}"].append(np.min(_v))
                        results[f"information_value_rnd_max_{score_name}"].append(np.max(_v))
                    if "_self" in k:
                        score_name = k[: -len("_self")]
                        results[f"expected_rnd_information_value_{score_name}"].append(np.mean(_v))
                        results[f"information_value_rnd_deviation_{score_name}"].append(
                            abs(
                                results[f"information_value_rnd_mean_{score_name}"][-1]
                                - results[f"expected_rnd_information_value_{score_name}"][-1]
                            )
                        )
        if args.logging:
            logger.warning(
